...
Разброс `D(z)-z` и самих `D(z)`, попарное cosine distance, inverse SNR,
направление score относительно отрицательного steering-вектора, ортогональная
steering-компонента score, raw L2 ошибка denoiser и изменение проекции на
steering-вектор агрегируются в графики и сохраняются на уровне токенов для
интерактивной HTML-подсветки. Формулы и запуск описаны в
