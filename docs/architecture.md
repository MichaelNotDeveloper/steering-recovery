...
Разброс `D(z)-z` и самих `D(z)`, попарное cosine distance, inverse SNR, raw L2
ошибка denoiser и изменение проекции на steering-вектор агрегируются в графики и
сохраняются на уровне токенов для интерактивной HTML-подсветки. Формулы и запуск
описаны в [отдельном документе](epistemic_steering.md).
