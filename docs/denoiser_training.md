...
Всего одновременно обучаются 36 независимых моделей. Они находятся в памяти
...
Отдельный preset для epistemic-эксперимента оставляет только четыре модели:

```bash
python train_denoiser.py experiment=epistemic_dropout
```

Все четыре имеют `latent_dim=3072`, три residual block и `dropout=0.1`; различается
только `sigma`: `0.1`, `0.2`, `0.5`, `1.0`. Подробный второй этап эксперимента описан в
