...
Каталог `data/` не включён в Git. После клонирования векторы нужно получить
командой выше; это гарантирует, что они построены текущей версией FP32-пайплайна.
Данные, checkpoints и результаты запусков остаются локальными артефактами.
